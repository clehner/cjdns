 ...
 * bundling libevent or openssl or some other megalyth.
 *
 ...
 * feedbck loop design leads to issues like:
 ...
 * Random_SeedGen.collectedEntropy, critical to the design is that Random_SeedGen.seed
 * is never touched. This means even if Random_addRandom() is so deeply flawed that it causes
 * collectedEntropy to become predictable, there is still 256 bits of primary random material
 * from the operating system in Random_SeedGen.seed which is never touched as long as the
 * application runs.
 *
 * Every 256 calls to Random_addRandom(), Random_SeedGen is again hashed into random.tempSeed.
 * Since collectedEntropy is a 256 bit buffer, each call should be worth at least 1 bit.
 * Note that Random.nonce is *not* reset ever during the operation of the generator because
 * otherwise, 512 successive calls to Random_addRandom() with the same input would lead to
 * the random generator duplicating it's output.
 */
